...
PanelWindow {
    ...
    WlrLayershell.namespace: "screenshot-browser"
    ...
    function buildScanCmd() {
        return ["bash", "-c",
            ...
            "find \"$D\" -maxdepth 1 -type f -iname '*_hyprshot.png' " +
    ...
    readonly property string scanCachePath: Quickshell.env("HOME") + "/.cache/quickshell-scan-screenshots"
    ...
    Process {
        ...
        onExited: function(code) {
            if (code !== 0) { console.warn("ScreenshotBrowser: trash failed (gio/trash-put unavailable) — file KEPT, not deleted"); return }
    ...
    function copyFocused() {
        if (!sel || !sel.filePath) return
        copyProc.command = ["bash", "-c", "wl-copy --type image/png < " + shq(sel.filePath)]
        copyProc.running = false; copyProc.running = true
    }

    function mediaLabel(path) {
        var n = String(path || "").split("/").pop().replace(/\.[^.]+$/, "")
        var m = n.match(/(\d{4})-(\d{2})-(\d{2})[-_]?(\d{2})[-_]?(\d{2})[-_]?(\d{2})/)
                ...
                Rectangle {
                    ...
                    Rectangle {
                        ...
                        Rectangle {
                            anchors.fill: parent
                            color: panel.paper
                            opacity: item.focused ? 0 : (Math.abs(item.relIdx) === 1 ? 0.34 : 0.55)
                            Behavior on opacity { NumberAnimation { duration: 200 } }
                        }
                        Rectangle {
                            anchors.fill: parent
    ...
    Column {
        ...
        Text {
            ...
            text: "Screenshots · " + panel.currentLabel
            color: panel.ink
        ...
        Text {
            ...
            anchors.horizontalCenter: parent.horizontalCenter
            text: "Delete this screenshot?   Del again to confirm   ·   Esc cancel"
            color: panel.accent
            font.family: panel.mono; font.pixelSize: 11; font.weight: Font.Medium
            horizontalAlignment: Text.AlignHCenter
        }
