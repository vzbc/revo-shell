...
layout(binding = 3) uniform sampler2D bloomSource;

layout(std140, binding = 0) uniform buf {
    mat4  qt_Matrix;
    float qt_Opacity_dummy;
    vec2  iResolution;

    vec4  glassRefraction; // Refraction parameters (dim, mag, aberration, PADDING)
    vec4  glassRefractionIOR;// IOR values for red, green, blue

    vec4  glassEdgeDim; // Edge area size
    vec4  glassTintColor; // Tint color
    vec4  blurAmount; // Blur

    vec4  rimLightDir; // Rim light angle
    vec4  rimLightColor; // Rim light color

    vec4 reflectionOffset; // min mag

    vec4 fieldSmoothing; // Smoothing of nearby shapes
    vec4 shape; // x y w h
    vec4 shapeExt; // radius rot type PADDING
};
...
#define SAMPLES blurAmount.r   // original kernel width in full-res pixels
...
vec3 field(vec2 fragCoord)
{
    ...
    vec2 centerPos = shape.xy + 1 * shape.zw;

    // shapePos is now in pixels from screen center
    vec2 lp = rot(p - centerPos, -radians(shapeExt.y));

    vec3 d;
    if (shapeExt.z == 0)
        d = sdgCircle(lp, shape.z);              // radius in px
    else if (shapeExt.z == 1)
        d = sdgBox(lp, shape.zw, shapeExt.x);     // size + radius in px
...
void refractionLayer(
    ...
    float boundary = lerp(-glassRefraction.r, EPS, d);
    boundary = mix(boundary, 0., smoothstep(0., EPS, d));

    float cosBoundary = 1.0 - cos(boundary * PI / 2.0);
    float interior = smoothstep(EPS, 0., d);

    vec3 ior = mix(vec3(glassRefractionIOR.g), glassRefractionIOR.rgb, glassRefraction.b);
    vec2 pxToUV = 1.0 / iResolution;

    vec2 offset = -norm * glassRefraction.g * pxToUV;
...
void tintLayer(
    ...
    float a = interior;
    float b = lerp(-glassEdgeDim.r, 0., d);
    float edge = min(a, b);

    float cosEdge = 1. - cos(edge * PI / 2.);

    float rimLightIntensity = abs(dot(normalize(norm), rimLightDir.xy));
    vec3 rimLight = rimLightColor.rgb * rimLightColor.a * rimLightIntensity;

    vec2 pxToUV = 1.0 / iResolution;
    vec2 _reflectionOffset =
        (reflectionOffset.x + reflectionOffset.y * cosEdge)
        * norm * pxToUV;

    vec3 reflectionColor = clamp(texture(bloomSource, UV + _reflectionOffset).rgb, 0., 1.);
