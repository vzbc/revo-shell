...
Item {
    ...
    property color surfaceColor: Qt.rgba(255, 255, 255, 0.15)
    ...
    // A conservative blur-only rectangle inside the natural SDF neck.
    function neckBlurShape(index) {
        ...
        const halfHeight = Math.max(0, Math.min(radius, Math.sqrt(Math.max(0, reach * reach - distance * distance / 4))) - root.blurEdgeInset);
